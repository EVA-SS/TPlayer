...
namespace TSkin
{
    public static class GDIEx
    {
        ...
        public static GraphicsPath CreateRoundedRectanglePath(this Rectangle rect, int radius, UICornerRadiusSides radiusSides)
        {
            ...
            {
                Point[] points = new Point[] { new Point(rect.X, rect.Y), new Point(rect.Width + rect.X, rect.Y), new Point(rect.Width + rect.X, rect.Height + rect.Y), new Point(rect.X, rect.Height + rect.Y), new Point(rect.X, rect.Y), };
                path = points.Path();
            }
            else
            {
        ...
        public static GraphicsPath CreateRoundedRectanglePath(this RectangleF rect, float radius, UICornerRadiusSides radiusSides)
        {
            GraphicsPath path;

            if (radiusSides == UICornerRadiusSides.None || radius == 0)
            {
                PointF[] points = new PointF[] { new PointF(0, 0), new PointF(rect.Width, 0), new PointF(rect.Width, rect.Height), new PointF(0, rect.Height), new PointF(0, 0), };
                path = points.Path();
            }
            else
            {
        ...
        public static GraphicsPath CreateRoundedRectanglePath(this RectangleF rect, float radius, bool cornerLeftTop = true, bool cornerRightTop = true, bool cornerRightBottom = true, bool cornerLeftBottom = true)
        {
            GraphicsPath path = new GraphicsPath();
            if (cornerLeftTop)
                path.AddArc(rect.X, rect.Y, radius, radius, 180, 90);
            else
                path.AddLine(new PointF(rect.X, rect.Y + 1), new PointF(rect.X, rect.Y));

            if (cornerRightTop)
                path.AddArc(rect.X + rect.Width - radius, rect.Y, radius, radius, 270, 90);
            else
                path.AddLine(new PointF(rect.X + rect.Width - 1, rect.Y), new PointF(rect.X + rect.Width, rect.Y));

            if (cornerRightBottom)
                path.AddArc(rect.X + rect.Width - radius, rect.Y + rect.Height - radius, radius, radius, 0, 90);
            else
                path.AddLine(new PointF(rect.X + rect.Width, rect.Y + rect.Height), new PointF(rect.X + rect.Width, rect.Y + rect.Height));

            if (cornerLeftBottom)
                path.AddArc(rect.X, rect.Bottom - radius, radius, radius, 90, 90);
            else
                path.AddLine(new PointF(rect.X + 1, rect.Y + rect.Height), new PointF(rect.X, rect.Y + rect.Height));
