...
{
    public partial class VideoList : Control
    {
        ...
        #region 属性
        public List<VideoListItem> Items = new List<VideoListItem>();
        ...
        public bool Kdown(Keys KeyCode)
        {
            ...
            {
                VideoListItem TitleItem = Items[SelectIndex];
                if (TitleItem != null)
                {
        ...
        void upha(int seb)
        {
            int bbqq = seb -= 1; try
            {
                VideoListItem TitleItem = Items[bbqq];
                while (TitleItem != null)
                {
                    if (TitleItem.Visible)
                    {
                        ...
                        foreach (VideoListItem TitleItems in Items)
                        {
        ...
        public void SetSv(VideoListItem TitleItem)
        {
        ...
        public void SetSvs(VideoListItem TitleItem)
        {
            if (chatVScroll.ShouldBeDraw)
            {
        ...
        public void InPaint()
        {
            if (Width > 0 && Height > 0)
            {
                ...
                try
                {
                    foreach (VideoListItem it in Items)
                    {
                        if (it.Visible)
                        {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            List<VideoListItem> _Items = Items.FindAll(db => db.Visible && (db.Bounds.Y > tom && db.Bounds.Y - toms < Height));
            try
            {
                foreach (VideoListItem it in _Items)
                {
                    if (_IsCheck)
                    {
        ...
        public delegate void DownEventHandler(VideoListItem Item);
        ...
        Point old;
        public void Tom(VideoListItem its)
        {
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            else
            {
                ...
                else
                {
                    try
                    {
                        foreach (VideoListItem its in Items)
                        {
                            if (its.Visible)
                            {
                                Rectangle rect = its.Bounds;
        ...
        public VideoListItem SelectItem { get; set; }
    }
    ...
    public class VideoListItem
    {
        ...
        public string BPin { set; get; }
        public string SPin { set; get; }
